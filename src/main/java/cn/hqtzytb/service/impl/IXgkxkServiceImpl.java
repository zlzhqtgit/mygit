...
public class IXgkxkServiceImpl implements IXgkxkService {
	...
	public ResponseResult<List<Vocation>> getVocationList(String industryName, HttpServletRequest request) {
		try {
			...
			return new ResponseResult<List<Vocation>>(ResponseResult.STATE_OK,Constants.RESULT_MESSAGE_SUCCESS,vocationList);
		} catch (Exception e) {
	...
	public ResponseResult<List<Specialty>> getSpecialtyList(String vocationId, HttpServletRequest request) {
		try {
			List<Specialty> specialtyList = specialtyMapper.selectvoction(" e.vocation_id = '" + vocationId + "' ", null, null, null);
			return new ResponseResult<>(ResponseResult.STATE_OK,specialtyList);
		} catch (Exception e) {
...
//		}
	...
	public ResponseResult<Void> handleCpXk(HttpServletRequest request) {
		...
		try{
			...
			if(subject.isAuthenticated()){
				Session session = subject.getSession();
				List<UserFeature> featurelist=userFeatureServer.getUserFeatureByUid((Integer)session.getAttribute("uid"));
