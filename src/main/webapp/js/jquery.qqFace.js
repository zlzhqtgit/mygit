	...
	$.fn.qqFace = function(options){
		...
		$(this).click(function(e){
			...
			if($('#'+id).length<=0){
				...
				for(var i=1; i<=75; i++){
					labFace = '['+tip+i+']';
					strFace += '<td><img src="'+url+i+'.gif" onclick="$(\'#'+option.assign+'\').setCaret();$(\'#'+option.assign+'\').insertAtCaret(\'' + labFace + '\');" /></td>';
...
function pathUrl(){
	 ...
	 var url=localhostPaht+projectName+'/img/public/emotions/';
	 return url;
}
...
function replace_em(str){
	...
	str = str.replace(/\>/g,'&gt;');
	str = str.replace(/\n/g,'<br/>');
	str = str.replace(/\[em_([0-9]*)\]/g,'<img src="'+pathUrl()+'/$1.gif" border="0"/>');
	return str;
}