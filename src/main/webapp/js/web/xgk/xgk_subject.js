...
//新高考学科探索的页面js
	...
	function jztm (nextnum) {
		if (nextnum<total) {
			...
			var url="../sub/xgk_potential_analysis.do";
			//保存潜能测评信息
	...
	function analysis() {
		...
		for(var i=0;i<total_sc.length;i++){
			if(total_sc[i][1]=="自信"){
				tr += '<td>'+total_sc[i][2]+'</td>';
			}else if(total_sc[i][1]=="兴趣"){
				tr1 += '<td>'+total_sc[i][2]+'</td>';
			}else{
				tr2 += '<td>'+total_sc[i][2]+'</td>';
			}
		}
		tr = tr+"</tr>" +tr1+"</tr>"+tr2+"</tr>";
		$("#cptbody").html(tr);

		var myChart1 = echarts.init(document.getElementById('major1'));
		option.title.text = "综合分析";
